...
fn main() {
    ...
    'values: for value in &picked_values {
        if bingo_grids.len() == 1 {
            let bingo_grid = bingo_grids.get_mut(0).unwrap();
            bingo_grid.check_a_number(*value);
            if bingo_grid.has_grid_won() {
                println!("{} {}", value, bingo_grid.calculate_sum_of_unmarked());
                println!("{}", bingo_grid.calculate_sum_of_unmarked()*value);
                break 'values
            }
        }
        else {
            for index in 0..bingo_grids.len() {
                let bingo_grid = bingo_grids.get_mut(index).unwrap();
                bingo_grid.check_a_number(*value);
            }
            bingo_grids.retain(|x| !x.has_grid_won())
        }
    }
}
